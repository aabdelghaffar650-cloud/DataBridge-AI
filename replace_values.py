...
def render(df) -> None:
    ...
    with tab3:
        st.markdown('<div class="info-box">Create a new column based on a formula using existing columns.<br>Example: <code>Price * 0.14</code> to calculate 14% tax. Use column names as variables.</div>', unsafe_allow_html=True)
        ...
        if st.button("➕ Add Computed Column"):
            if new_col_name and formula:
                ...
                try:
                    ...
                    result = eval(formula, {"__builtins__": {}}, {**local_vars, "pd": pd, "np": np})
