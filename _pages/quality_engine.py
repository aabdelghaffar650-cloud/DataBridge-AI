...
def render(df: pd.DataFrame) -> None:
    ...
    with q_tab1:
        if _dup_count == 0:
            ...
            # Use the lightweight sample (max 20 rows) stored in the report.
            # Full duplicate rows are NOT kept in session to avoid RAM doubling.
            dup_sample = qr.get("duplicate_sample", pd.DataFrame())
            truncated  = qr.get("duplicate_indices_truncated", False)
            if not dup_sample.empty:
                label = f"Showing first {len(dup_sample)} of {_dup_count:,} duplicates" if truncated else f"All {_dup_count:,} duplicate rows"
                st.caption(label)
                st.dataframe(dup_sample, use_container_width=True, height=300)
